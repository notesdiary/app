...
- `syncFilterRule(id)`: verify connection (`needsReauth` check) → `ensureDriveFolderId()` → if no known `driveFileId`, self-heal by listing Drive files in the folder named `<fileName>.json` (handles files created from another device) → if found, `files.status(driveFileId)` checks `changedSinceRestore` against this client's last read/write of the file: **unchanged** → write local matches as-is (no merge — lets local deletions take effect instead of being resurrected from the still-present remote copy); **changed** (or just self-healed, i.e. never restored here) → read remote JSON, union with local matches by `id` (local wins on collision), persist remote-only entries locally, write merged array back → if no file found at all, create one with local matches. Updates `filterSyncState[id]` to `{status: 'synced', lastSynced, driveFileId}`.
