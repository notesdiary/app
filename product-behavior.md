...
- **Pulling an existing backup on a new browser/device**: connecting Drive, and "Sync filters now", both first download *every* JSON backup file in this project's Drive folder — including files whose filenames match no local filter rule (filter rules are stored locally, so a project re-created under the same name on another device only knows its auto-seeded remainder rule). Downloaded entries are merged by `id` with local entries winning; no filter rule is created for an unreferenced file, so nothing is written back to it from this browser. Per-row "Sync now" does not do this folder-wide pull — it only self-heals its own filename.
- **Sync direction per row**: if the Drive file hasn't changed since this browser's last sync of it, "Sync now" writes local matches as-is (deleting/archiving an entry locally and re-syncing removes it from Drive too). If the Drive file changed since — an edit from another device, or a first-time sync — local and remote are merged by `id` (local wins on collision) instead of a plain overwrite, so a concurrent edit from elsewhere isn't clobbered.
