...
describe('drive.ts folderPath wiring', () => {
     ...
     *
     * Rationale: a wrong folderPath value does NOT throw or error anywhere. It
     * silently creates a brand new, empty Drive folder, and every existing
     * user's backup simply appears to have vanished from the app's point of
     * view. That failure mode is invisible without a test pinning the exact
     * value.
     */
    it('ensureFolderPath() walks exactly ["Notes Diary"]', async () => {
        ...
        expect(uniqueInOrder).toEqual(['Notes Diary']);
    });

    /**
     * `ensureProjectFolderId` restores the pre-migration folder layout on top
     * of the library's app-wide-only `folderPath`: a per-project subfolder
     * nested under "Notes Diary" for normal projects, and the top-level
     * folder directly for the legacy migrated project (`isLegacyProject`).
     */
    it('ensureProjectFolderId() nests a per-project subfolder under "Notes Diary" for a normal project', async () => {
        const { ensureProjectFolderId } = await import('../lib/drive');
        ...
        await ensureProjectFolderId('proj-1', 'My Cool Project', false);

        const uniqueInOrder = [...new Set(driveFake.folderCreationLog)];
        expect(uniqueInOrder).toEqual(['Notes Diary', 'My Cool Project']);
    });

    it('ensureProjectFolderId() uses the top-level folder directly for the legacy migrated project', async () => {
        const { ensureProjectFolderId } = await import('../lib/drive');
        ...
        await ensureProjectFolderId('legacy-proj', 'Legacy Project Name', true);

        const uniqueInOrder = [...new Set(driveFake.folderCreationLog)];
        expect(uniqueInOrder).toEqual(['Notes Diary']);
    });

    it('two different projects on the same account resolve to two different folder ids', async () => {
        const { ensureProjectFolderId } = await import('../lib/drive');
        ...
        const folderA = await ensureProjectFolderId('proj-a', 'Project A', false);
        ...
        const folderB = await ensureProjectFolderId('proj-b', 'Project B', false);
